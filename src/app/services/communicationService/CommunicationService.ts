...
import { TestDbState, TableData, MainTableRow, DataType, TField, FieldType } from "../../../Database/Redux/TestDB/types/DataTypes";
import { addTable } from "../../../Database/Redux/TestDB/actions/tableActions";
...
class CommunicationService {
    ...
    public createAvgSummaryTable(tableName: 'main', characteristicId: string, metricId: string): string {
    ...
    private createAvgSummaryTableData(tableName: 'main', characteristicId: string, metricId: string): TableData {
        const metricValues: Map<string, number[]> = new Map();
        const rows: MainTableRow[] = this.store.getState().tables.byId[tableName].values;
