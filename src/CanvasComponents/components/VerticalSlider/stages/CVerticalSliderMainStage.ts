...
export class CVerticalSliderMainStage extends AbstractCanvasStage {
    ...
    protected createLayers(): void {
        const baseLayer: ILayer = new CVerticalSliderBaseLayer(this, this.model, (_layer) => this.getMainStageLayerParams());
        this.addLayer(baseLayer);

        const btnDimensions: TDimensions = this.getButtonDimensions();

        const upBtn: ILayer = new CVerticalSliderButtonLayer(this, this.model, () => ({ dX: 0, dY: 0, ...btnDimensions }), { direction: Direction.Up, callback: () => console.log("xD") });
        this.addLayer(upBtn);

        const borderWidth: number = this.painter.getSliderBorderWidth(this.layerWidth);

        const trackStage: ILayer = new CVerticalSliderTrackStage(this, this.model, (_layer) => ({
            dX: borderWidth,
        ...
        const downBtn: ILayer = new CVerticalSliderButtonLayer(this, this.model, () => ({ dX: 0, dY: this.layerHeight - btnDimensions.height, ...btnDimensions }), { direction: Direction.Down, callback: () => console.log("xD") });
